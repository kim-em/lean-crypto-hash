...
## Security Note
SHAKE256 is cryptographically secure and provides variable-length output.
The security level depends on the output length chosen.

## Usage
```
shake256sum -l 32 < input.txt      # 32 bytes (256 bits) output
shake256sum --length 64 file.txt   # 64 bytes (512 bits) output
```
-/
...
def main (args : List String) : IO Unit := do
  if args.contains "--help" || args.contains "-h" then
    IO.println "Usage: shake256sum [OPTIONS] [FILE]..."
    IO.println "Generate SHAKE256 hash with variable output length"
    IO.println ""
    IO.println "Options:"
    ...
    IO.println ""
    IO.println "Examples:"
    IO.println "  echo 'hello' | shake256sum -l 16"
    IO.println "  shake256sum --length 64 file.txt"
    return
